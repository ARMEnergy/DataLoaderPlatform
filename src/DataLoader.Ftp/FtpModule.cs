...
public sealed class FtpWorkUnitProvider : IWorkUnitProvider<FtpFileWorkUnit>
{
    ...
    public FtpWorkUnitProvider(IFtpFileSystem fs, IOptions<FtpSettings> settings, ILogger<FtpWorkUnitProvider> logger)
    {
...
public sealed class FtpSourceReader : ISourceReader<FtpFileWorkUnit, FtpFeedRow>
{
    ...
    public FtpSourceReader(IFtpFileSystem fs, IOptions<FtpSettings> settings, ILogger<FtpSourceReader> logger)
    {
        _fs = fs; _settings = settings.Value; _logger = logger;
    }
...
public sealed class FtpSqlSink : SqlSinkBase<FtpFeedRow>
{
    ...
    public FtpSqlSink(IOptions<FtpSettings> settings, ILogger<FtpSqlSink> logger)
        : base(logger)
    {
...
/// </summary>
public sealed class FtpModule : ILoaderModule
{
    public const string Id = "Ftp";
    ...
    {
        services.Configure<FtpSettings>(configuration.GetSection($"Loaders:{Id}"));
        ...
        services.AddSingleton<FtpPipeline>();
    }

    public async Task<LoaderRunResult> RunAsync(IServiceProvider services, LoaderRunContext context)
    {
        var pipeline = services.GetRequiredService<FtpPipeline>();
        return await pipeline.ExecuteAsync(context).ConfigureAwait(false);
    }
}

public sealed class FtpPipeline : LoaderPipelineBase<FtpFileWorkUnit, FtpFeedRow, FtpFeedRow>
{
    public FtpPipeline(
        ...
        ILogger<FtpPipeline> logger)
        : base(FtpModule.Id, workUnits, source, transformer, sink, loadLog, settings.Value, logger)
    {
    }
}
