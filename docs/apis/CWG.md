...
column of all **15 endpoints** is enumerated below with its exact source name, ordered
...
|-------|-------------|-----------|
| **A** | Simple tabular, one row = one record | CityForecast, CityGasForecast, CityObservation, Station, NationalDegreeDays |
...
| **Weighted degree-days (decimal)** | NationalDegreeDays NG/POP/ELEC HDD/CDD | **`DECIMAL(9,4)`** | values < ~100, 4 dp |
...
- **`{units}`** = **`F` only** (locked scope; `C` exists but is not loaded).
...
| 4 | `Fcst Mn` | `DECIMAL(5,1)` | No | forecast min temp °F (integer-valued in samples). Header is literally `Fcst Mn` |
| 5 | `Fcst Mx` | `DECIMAL(5,1)` | No | forecast max temp °F. Header is literally `Fcst Mx` |
| 6 | `Fcst Avg` | `DECIMAL(5,1)` | No | forecast avg temp °F (has `.5` values, e.g. `72.5`) |
| 7 | `Norm Mn` | `DECIMAL(5,1)` | No | normal min temp °F (1 dp). Header is literally `Norm Mn` |
| 8 | `Norm Max` | `DECIMAL(5,1)` | No | normal max temp °F (1 dp). **Header quirk: `Norm Max` (spelled out), asymmetric with `Norm Mn`** |
| 9 | `HDD` | `SMALLINT` | No | heating degree-days (integer) |
| 10 | `CDD` | `SMALLINT` | No | cooling degree-days (integer) |

Natural key: `region + Station + ProductionDate + Date` (units fixed to `F`).
...
- **`{subregion}`** = **`national` ONLY** (all other sub-region guesses 404).
- **`{date}`** = `YYYYMMDD` (today valid).
...
Natural key: `RunDate (from filename) + DATES`. (`subregion` fixed to `national`.)
...
## Coverage checklist (all 15 endpoints — column list + type + nullability + shape)
...
| 8 | NationalDegreeDays | A | 14 | `national` only; `IS_FORECAST` True/False→BIT; digit-leading headers |
...
**Gate status: PASS** — all 15 endpoints have a complete ordered column list, a
