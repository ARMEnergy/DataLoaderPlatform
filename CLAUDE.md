...
Automated tests live under `tests/` (xUnit): `DataLoader.Core.Tests`, `DataLoader.Vulcan.Tests`, `DataLoader.Platts.Tests`, `DataLoader.StormVista.Tests`, `DataLoader.CWG.Tests`, `DataLoader.AGSI.Tests`, `DataLoader.IHSPointLogic.Tests`, `DataLoader.IIR.Tests`, `DataLoader.OPIS.Tests`, `DataLoader.NGI.Tests`, and `DataLoader.ModernCommodities.Tests`. Run them with:

```bash
dotnet test DataLoaderPlatform.sln -c Release
```

**The build never touches `sql/`.** No `.sql` script is compiled or executed by
`dotnet build`/`dotnet test`, and most recent loaders are build-only (SQL never
deployed), so a green solution says *nothing* about whether the SQL even parses —
a T-SQL syntax error can sit in a shipped script until someone runs it, and one
did (`sql/ModernCommodities/003`, an attempted `(a = x) <> (b = x)` XOR; T-SQL has
no boolean value type). Parse-check without a server or a connection —
`sqlcmd`/`sqlpackage` are not installed here:

```bash
dotnet new console -o sqlcheck && cd sqlcheck
dotnet add package Microsoft.SqlServer.TransactSql.ScriptDom
# TSql160Parser(false).Parse(reader, out IList<ParseError> errors) per *.sql
dotnet run -- ../sql/<Vendor>
```

All 44 scripts under `sql/` parse clean as of 2026-08-25. This catches **syntax
only** — a missing object, a TVP whose columns drifted from its table, or a type
mismatch still needs a real deployment.
...
- **Per-loader DB**: each loader owns its own schema (e.g., `ea` for EnergyAspects; `arm` for Platts, CWG, AGSI, IHSPointLogic, IIR, OPIS, NGI, and ModernCommodities — the `arm` name is reused but each lives in its own database) — though `StormVista` deliberately uses the default **`dbo`** schema rather than a vendor prefix, per the `DATABASE_DEVELOPER` agent's own standing convention ("target schema is `dbo` unless the task says otherwise"). Scripts in `sql/<Vendor>/`. Connection string comes from `Loaders:<LoaderId>:ConnectionString`.
...
("What Each Project Contains") has longer write-ups but predates AGSI, IHSPointLogic, IIR, OPIS, NGI and ModernCommodities:
...
- `DataLoader.ModernCommodities/` — **HTTPS+CSV** (Modern Commodities / ModCom — crude & refined trade and settlement data), the platform's **first CSV-over-HTTP loader using HTTP Basic auth**, and the first whose fact tables carry **no `FileLogId`**. Three **closed, fully INDEPENDENT pipelines** over `LoaderPipelineBase`, one per endpoint: `allTrades/v1` → `arm.AllTrades` (PK `TradeNumber`), `myTrades/v1` → `arm.MyTrades` (PK `TradeNumber`), `settlements/v1` → `arm.Settlements` (PK `(SettlementDate, Product, Location, PieplineTerminal, PriceBasis, Term)`). No reference provider, no barrier, no FK, no discovery tier — any subset of endpoints is a valid run. **Responses are `text/csv`, so `HttpJsonSourceReaderBase` is forbidden** (its `EnsureSuccessStatusCode` would also throw before the error body could be classified); the RFC4180 tokenizer is adapted from `CwgCsv` minus CWG's `END.` terminator. **The defining data fact: the trades date window filters on `Last Updated Timestamp`, not `Executed Timestamp`** — verified, 5 of 98 rows in a 2026-08-20..24 pull were executed as far back as 2026-08-06 while *zero* rows fell outside the window on last-updated. That is why the merge key is `TradeNumber` alone and why the rolling 30-day re-pull is what captures revisions and Finalized→Cancelled flips, and therefore why the resume key is **hot-only at hour granularity** (`yyyyMMddHH`, IHS `RunHour` style, **no settled zone at all** — nothing here is ever final, so a stable key would freeze stale rows forever). `allTrades` and `myTrades` share one byte-identical **34-column** header, one `TradeRow`, one `BuildTable` and one `arm.TradesTvp`; only the sink differs. The vendor **anonymises 14 counterparty columns in `allTrades`** (`Side`, the four `Bid*`/four `Offer*`, `SettlementCurrency`, `ContractTerms`, `GTandC`, `Notes`, `ClickAndTrade`) that `myTrades` populates — which is the whole reason two tables exist; the full 34 columns stay on both and the validator treats those NULLs as **informational**. `ClearingID` is populated by *neither* endpoint, so nothing may assert it. Four parse traps guarded centrally: timestamps are **12-hour `yyyy-MM-dd hh:mm:ss tt`** with no offset (an `HH` pattern or a bare `DateTime.Parse` silently mangles every afternoon row — stored unshifted), fields are quoted with **legitimate embedded commas** (a `Split(',')` turns one address into six fields), header names contain spaces/`&`/`/` (`Pipeline/Terminal`, `GT&C`, `Click & Trade`) so binding is by **literal name** via an ordinal map, and settlements uses the literal string **`-`** for an absent `Location`/`PipelineTerminal` which must survive **verbatim** because it sits inside the `NOT NULL` PK — so key fields get a *narrower* sentinel policy than measures, and an unkeyable row is dropped-and-counted. **Two distinct 400s that mask each other**: `startDate must be within the last six months` and `Request returns more than the limit of 10000 rows` — the history limit is exactly **6 calendar months** (`today.AddMonths(-6)`; verified 2026-02-24 accepted / 2026-02-23 rejected), *not* 183 days, and the row cap is genuinely reachable (allTrades ≈58 rows/day trips 10,000 at ~172 days; settlements ≈721 rows per *published* date makes a 6-month pull ~94% of its 100,000 cap), so `ChunkDays` is a real requirement for any deep backfill. A **header-only 200 is a successful empty read**; every non-2xx throws (the opposite of NGI, where 404 is routine). Trades `WHEN MATCHED` carries an OPIS-style **recency guard** (`tgt.LastUpdated IS NULL OR (src.LastUpdated IS NOT NULL AND src.LastUpdated >= tgt.LastUpdated)`) with in-batch dedup ordered `LastUpdated DESC`, so concurrent/out-of-order chunks and re-runs cannot regress a revision. Deliberate departures from house shape, all at the user's authoritative DDL: **no surrogate `Id`, no `DateCreated`, no `FileLogId`, no `Checksum`**, so the TVPs start at the first payload column and provenance lives in `arm.FileLog` alone (kept per-pull, not upserted, precisely because it is the only lineage); `PieplineTerminal` is **misspelled in the user's PK and reproduced verbatim** in table, TVP, proc *and* the C# property. `DaysBack=30` yields an inclusive **31-day** window on purpose. `arm` schema (DB `ModernCommodities`), `arm.usp_ValidateLoad`, guarded `999` teardown. Build-only (disabled); **all three endpoints, Basic auth, both 400 bodies, the 401, the calendar-month boundary and the last-updated filter were confirmed against the live API with zero reconstructed fields**, but the SQL has never been deployed and no load has run. See also `docs/apis/ModernCommodities.md`, `docs/design/ModernCommodities.md`, `sql/ModernCommodities/`.
...
- SQL changes go through `DATABASE_DEVELOPER`; C# through `CODER`; both are
  ...
  New or edited SQL is **parse-checked** (see [Build and Run](#build-and-run)) —
  green tests do not cover it.
- If a stage is cut short (an agent hits a spend or context limit, a tool fails),
  that stage **did not run**. Name it as still owed and say what you verified by
  other means instead; never let an interrupted stage read as a clean pass.
...
- Report the actual `dotnet build` / `dotnet test` result. "Build-only" loaders
  (CWG, AGSI, IHSPointLogic, IIR, NGI, ModernCommodities) are compiled and unit-tested but have never been
...
- `tests/` — xUnit test projects, one per loader (`DataLoader.Core.Tests`, `DataLoader.Vulcan.Tests`, `DataLoader.Platts.Tests`, `DataLoader.StormVista.Tests`, `DataLoader.CWG.Tests`, `DataLoader.AGSI.Tests`, `DataLoader.IHSPointLogic.Tests`, `DataLoader.IIR.Tests`, `DataLoader.OPIS.Tests`, `DataLoader.NGI.Tests`, `DataLoader.ModernCommodities.Tests`)
- `docs/apis/` + `docs/design/` — per-loader field reference and flow spec (the current per-loader record)
- `docs/ARCHITECTURE.md` — design rationale (predates AGSI / IHSPointLogic / IIR / OPIS / NGI / ModernCommodities)
